...
def create_app():
    ...
    # -----------------------------------------------------------
    #  Student Task Book (Takenboek Leerling)
    # -----------------------------------------------------------
    @app.route('/dashboard/<chef_naam>/takenboek')
    @login_required
    ...
    @app.route('/dashboard/<chef_naam>/takenboek/create', methods=['POST'])
    @login_required
    def create_takenboek(chef_naam):
        ...
        if not form.validate_on_submit():
            flash("Ongeldige CSRF-token.", "danger")
            return redirect(url_for('takenboek', chef_naam=chef_naam))
        ...
        try:
            ...
            if task_count > 0:
                flash("Je hebt al een takenboek. Je kunt handmatig taken toevoegen.", "warning")
                return redirect(url_for('takenboek', chef_naam=chef_naam))
            ...
            if not template:
                flash("Geen standaard takenboek sjabloon gevonden.", "danger")
                return redirect(url_for('takenboek', chef_naam=chef_naam))
            ...
            for match in matches:
                try:
                    blok, onderdeel, techniek, taak = match
                    # Unescape any escaped quotes in the task text
                    taak = taak.replace('\\"', '"')
                    ...
                    logger.warning(f"Error inserting task: {str(e)}, task: {match}")
                    continue
    ...
    @app.route('/dashboard/<chef_naam>/takenboek/add', methods=['GET', 'POST'])
    @login_required
    def add_task(chef_naam):
        ...
        return render_template('takenboek/add_task.html', chef_naam=chef_naam, form=form)

    @app.route('/dashboard/<chef_naam>/takenboek/edit/<int:task_id>', methods=['GET', 'POST'])
    @login_required
    def edit_task(chef_naam, task_id):
        ...
        try:
            ...
            if not task:
                flash("Taak niet gevonden of geen toegang.", "danger")
                return redirect(url_for('takenboek', chef_naam=chef_naam))
            
            if request.method == 'POST':
                if not form.validate_on_submit():
                    flash("Ongeldige CSRF-token.", "danger")
                    return redirect(url_for('edit_task', chef_naam=chef_naam, task_id=task_id))
    ...
    @app.route('/dashboard/<chef_naam>/takenboek/delete/<int:task_id>', methods=['POST'])
    @login_required
    ...
    @app.route('/admin/takenboek/trigger', methods=['GET', 'POST'])
    @login_required
            ...
            if request.method == 'POST':
                ...
                if template_id:
                    ...
                    cur.execute("""
                        ...
                        SET trigger_name = %s, trigger_sql = %s 
                        WHERE template_id = %s
                    """, (trigger_name, trigger_sql, template_id))
                    
                    flash("Trigger template bijgewerkt!", "success")
                else:
                    # Create new template
                    cur.execute("""
                    ...
                    flash("Nieuwe trigger template aangemaakt!", "success")
    ...
    @app.route('/admin/takenboek/trigger/<int:template_id>/set-default', methods=['POST'])
    @login_required
    ...
    @app.route('/admin/takenboek/trigger/<int:template_id>/delete', methods=['POST'])
    @login_required
    def delete_trigger(template_id):
        ...
        try:
            ...
            # Delete the template
            cur.execute("DELETE FROM task_trigger_template WHERE template_id = %s", (template_id,))
            
            conn.commit()
            flash("Trigger template verwijderd!", "success")
        
        except Exception as e:
            conn.rollback()
            logger.error(f'Error deleting trigger: {str(e)}')
            flash(f"Fout bij verwijderen trigger: {str(e)}", "danger")
